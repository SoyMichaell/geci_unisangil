...
SELECT * FROM prueba_saber_modulo
	...
	SELECT * FROM persona 
		...
		SELECT * FROM inv_investigador 
			...
			INNER JOIN inv_grupo_investigacion ON inv_investigador.inves_id_grupo = inv_grupo_investigacion.id
			WHERE persona.id = 57
			
			
			SELECT * FROM programa
			INNER JOIN programa_plan_estudio ON programa.id=programa_plan_estudio.pp_id_programa
			
			
			SELECT * FROM trabajo_grado
				INNER JOIN persona AS director ON trabajo_grado.tra_id_director=director.id
				INNER JOIN persona AS codirector ON trabajo_grado.tra_id_codirector=codirector.id