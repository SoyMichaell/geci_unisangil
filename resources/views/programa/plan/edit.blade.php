                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" name="pp_id_sede" id="pp_id_sede">
                            ...
                            @foreach ($municipios as $municipio)
                                <option value="{{ $municipio->id }}" {{$plan->pp_id_sede == $municipio->id ? 'selected' : ''}}>{{ $municipio->mun_nombre }}</option>
                            @endforeach
                        </select>
                        @error('pp_id_sede')
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" name="pp_id_programa" id="pp_id_programa">
                            ...
                            @foreach ($programas as $programa)
                                <option value="{{ $programa->id }}" {{$plan->pp_id_programa == $programa->id ? 'selected' : ''}}>{{ $programa->pro_nombre }}</option>
                            @endforeach
                        </select>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" name="pp_estado" id="pp_estado">
                            ...
                            <option value="inactivo" {{$plan->pp_estado == 'inactivo' ? 'selected' : ''}}>Inactivo</option>
                        </select>
                        @error('pp_estado')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
